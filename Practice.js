...
function getHello(str) {
 function getHello(arr) {
  var re = /hello/i;
  var helloArray = arr.filter(function (element) {
    return re.test(element);
  });

  helloArray.forEach(function (element) {
    console.log(element);
  });
}
...
function replaceTen(str) {
  var replacedStr = str.replace(/10/g, "Ten");
  return replacedStr;
}

var modifiedStr = replaceTen(strWith10);
...
function fixPostalCode(postalCode) {
  ...
  var postalCodeRegex = /^[ABCEGHJKLMNPRSTVXY]\d[A-Z] ?\d[A-Z]\d$/;
  if (!postalCodeRegex.test(postalCode)) {
    throw new Error("Invalid postal code");
  }

  postalCode = postalCode.replace(/[^A-Z\d]/, " ");
...
console.log(fixPostalCode("B2C3D4")); 
console.log(fixPostalCode("D1E2F3")); 
...
function toProvince(postalCode, useLongForm) {
  ...
  var provinceLookup = {
    ON: useLongForm ? "Ontario" : "ON",
    QC: useLongForm ? "Quebec" : "QC",
    NS: useLongForm ? "Nova Scotia" : "NS",
    NB: useLongForm ? "New Brunswick" : "NB",
    MB: useLongForm ? "Manitoba" : "MB",
    BC: useLongForm ? "British Columbia" : "BC",
    PE: useLongForm ? "Prince Edward Island" : "PE",
    SK: useLongForm ? "Saskatchewan" : "SK",
    AB: useLongForm ? "Alberta" : "AB",
    NL: useLongForm ? "Newfoundland and Labrador" : "NL",
    NT: useLongForm ? "Nunavut, Northwest Territories" : "NT",
    YT: useLongForm ? "Yukon" : "YT",
  };

  var firstLetter = postalCode.charAt(0);
  var province = provinceLookup[firstLetter];

  return province || null;
}

console.log(toProvince("M5W1E6"));
console.log(toProvince("M5W 1E6"));
console.log(toProvince("M5W 1E6", true));
console.log(toProvince("M5W"));
console.log(toProvince("X0A 1B2"));
console.log(toProvince("A1B 2C3", true));
...
function checkFirstChar(value) {
  ...
  var isFirstCharUppercase = /^[A-Z]$/.test(firstChar);
  var message = "String's first character is " + (isFirstCharUppercase ? "uppercase" : "not uppercase");
  return message;
}
...
console.log(validEmail("invalid-email"));
console.log(validEmail("test@123.45.67"));
